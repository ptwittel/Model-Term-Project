...
waste_inadequate_manage <-  waste_inadequate_manage %>% 
                            select(c("Entity", "Share of plastic inadequately managed","Code"))
```
...
gdp_country <- gdp_country %>% 
  rename("Entity" = `Country Name`,
         ...
         "Code" = `Country Code`) %>% 
  select(Entity, `GPD (in USD 2015)`, Code)

```

```{r}
tab1 <- full_join(waste_total, country_pop, by = "Entity")
tab2 <- full_join(tab1, waste_per_cap, by = c("Entity", "Code"))
tab3 <- full_join(tab2, waste_totgbl_mismanage, by = c("Entity", "Code"))
tab4 <- full_join(tab3, waste_inadequate_manage, by = c("Entity", "Code"))
tab5 <- left_join(tab4, gdp_country, by = c("Entity", "Code"))
...
tab7 <- left_join(tab5, fuel_sub, by = c("Entity", "Code"))
