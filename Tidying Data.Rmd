...
# gdp_country <- gdp_country %>% 
#   rename("Entity" = `Country Name`,
#          "GDP (in USD 2015)" = `2010`,
#          "Code" = `Country Code`) %>% 
#   select(Entity, `GDP (in USD 2015)`)
...
+ I think that we will have to rely on imputation for that. I doubt that we will be about to find good records on the missing data points. Though, I think that could be alright; I found a paper that talks about that effectiveness of imputation with different proportion of missing data. We have only about 30% missing, but the paper suggested that up to 40% is still effective. (This is with the MASSIVE caviot that we have MAR and not MI which I am not convince we have)
...
<<<<<<< HEAD
+I think we have legitimate reason to remove nations that were in political/social turmoil at or around that time. We don't have a means of accounting for the effect of plastic waste in the mist of a conflict. We would have to look at historical data to account for that effect which would be out of the scope of this report. 
=======
...
>>>>>>> bc1fec533198817c802a8a27fc44e6dc5ef41d98
...
+You are very right about that. I do not think that this should be apart of the main study, but I do think that there is value in studying this as a secondary thing that we might be about to comment on. though I recognize that to be difficult.
```{r}
