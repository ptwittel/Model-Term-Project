...
#summary(table_10) #no  more NAs

#write_csv(table_10, "Data Clean/imputedtable_10.csv")
```
...
```{r}
scope <- list(upper = ~ population + growth_rate + area + density + fert_rate + life_exp + mort_rate + per_cap_waste + misman_waste + share_misman + per_cap_gdp,
              lower = ~1)
...
```{r}
fit_1 <- with(data = imputed_data, expr = lm(waste_gen ~ area + misman_waste + per_cap_waste + population + share_misman))
...
```{r}
fit_without <- with(imputed_data, lm(waste_gen ~ area + misman_waste + per_cap_waste + population))
fit_with <- with(imputed_data, lm(waste_gen ~ area + misman_waste + per_cap_waste + population + share_misman))
D1(fit_with, fit_without)
```

```{r}
fit_2 <- with(data = imputed_data, expr = lm(waste_gen ~ area + misman_waste + per_cap_waste + population))
